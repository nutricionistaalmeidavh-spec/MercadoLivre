...
export async function recordMigration(env, sellerId, status) {
  await env.DB.prepare(`
    INSERT INTO migration_audit (seller_id, source, status, created_at) VALUES (?, 'vercel-cookie', ?, ?)
  `).bind(String(sellerId), String(status), Date.now()).run();
}
