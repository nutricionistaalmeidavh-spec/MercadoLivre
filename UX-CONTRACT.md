...
- Primary jobs: acompanhar estado da conta, localizar anúncios, configurar pós-venda, acompanhar pedidos e pagamentos, consultar/criar/encerrar promoções, publicar anúncios e diagnosticar o runtime.
...
| Saúde, configuração e atividade | `cloudflare/src/operations.mjs` | Runtime/API contract | 2026-09-16 |
...
- Runtime design-system/token source: `admin-cloudflare.html`, extensão `admin-operations.js` e console `admin-system.html`.
- Mapping/export/adapters: tokens CSS compartilhados por nome/valor e semântica.
...
| Diagnóstico | `/system` + `/api/operations` | runtime API | somente leitura | teste de segurança + fluxo manual |
...
| Status de configuração | label + Configurado/Ausente | n/a | n/a | n/a | n/a | n/a | nunca mostra valor secreto |
...
- Page size: anúncios até 100 ativos; pedidos até 40 na primeira página operacional; console mostra até 25 atividades por categoria por padrão.
- Empty/no-results/error/loading treatment: estados dedicados no mesmo espaço da lista, com skeleton ou indicador durante leitura remota.
- Back/scroll restoration: anúncio preserva posição; pedido retorna à lista sem perder filtros; console retorna para `Mais`.
...
| Diagnosticar sistema | Mais > Sistema e diagnóstico | botão busy | console | saúde/config/fila/logs atualizados | erro inline + atualizar | botão atualizar | `/api/operations` |
| Reconectar conta | Reconectar Mercado Livre | OAuth externo | callback/admin | sessão ML renovada | erro seguro no callback | painel | `/api/oauth` |
| Refresh | Atualizar | indicador | mesma view | dados atualizados | notice error | botão atualizar | APIs admin |
| Cancel/back | Voltar | n/a | lista ou Mais | contexto preservado | n/a | origem | client-side |
...
- Breadcrumb/tab/route-state policy: detalhe de anúncio usa tabs in-page; view principal usa query string; pedido usa subview com parâmetro `order`; `/system` é rota administrativa de segundo nível acessada por `Mais`.
...
- Responsive table strategy: cards independentes para anúncios, pedidos, promoções e logs.
...
- Session expiry/re-authentication: 401 retorna para login; `/system` redireciona para admin quando a sessão expira.
...
- Sensitive-value handling: senha mascarada; secrets nunca aparecem no painel; `/api/operations` retorna somente booleanos de configuração para credenciais.
...
- Permission UI strategy: sessão admin obrigatória em `/system` e `/api/operations`; 401 volta ao login.
- Clipboard copy policy: não há secrets copiáveis; callback/webhook são apenas leitura.
- Disabled-state explanation: operações promocionais não suportadas são exibidas como consulta somente leitura.
...
- Component-state/visual regression coverage: revisão manual do login, dashboard, anúncios, pedidos, promoções, sistema, detalhes e filtros.
...
- Failure-path evidence: login inválido, conta desconectada, falha de listagem, falha de pedido, rejeição de promoção, sessão expirada no console e D1 indisponível.
