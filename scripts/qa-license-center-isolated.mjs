...
const required=['obra.company.read','obra.company.create','obra.company.update-license','obra.device.control','debora.license.manage','owner.license.audit','loja-online.company.read','loja-online.company.create','loja-online.license.update','loja-online.license.extend','loja-online.license.block'];
...
try{
  ...
  for(const viewport of viewports){
    ...
    await page.route('**/api/license-center**',async route=>{
      ...
      if(pathname==='/api/license-center/debora/license'&&method==='POST'){
        ...
        if(action==='grant'){if(!row){row={email,status:'active',planCode:'pro_6m',expiresAt:'2027-03-25T00:00:00.000Z',source:'manual'};data.debora.clients.push(row)}else row.status='active';data.debora.overview.clients=data.debora.clients.length;return response(route,{state:'active',grant:row});}
    ...
    const debora=page.locator('#deboraLicenseForm');await debora.locator('[name=email]').fill(`qa-license-debora-${viewport.name}@example.test`);await debora.locator('[data-debora-action=grant]').click();await page.getByText(`qa-license-debora-${viewport.name}@example.test`,{exact:true}).waitFor();
  ...
  const report={status:'passed',generatedAt:new Date().toISOString(),viewports:results,checks:['refetch','qa_scope_violation','write_disabled','parity']};await fs.mkdir(path.dirname(outFile),{recursive:true});await fs.writeFile(outFile,JSON.stringify(report,null,2)+'\n');console.log(`LICENSE_CENTER_ISOLATED_REPORT=${outFile}`);
